...
type stdin = Stdin (** stdin of the parent process *)
type stdout = Stdout (** stdout of the parent process *)
type stderr = Stderr (** stderr of the parent process *)
type channel = Channel (** stream redirects to/from a channel *)
type devnull = Devnull (** stream redirects to /dev/null *)
type file = File of string (** stream redirects to a file which is
